  ...
  return (
    <section className="bg-bg px-6 md:px-16 py-[120px]">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-16 pb-8 border-b border-olive-wood-200">
        ...
        <p className="text-[15px] text-olive-wood-600 max-w-full md:max-w-[280px] leading-[1.6] mt-4 md:mt-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {PILARES.map(({ num, title, body, tag }) => (
          <div key={num} className="pt-9 border-t-2 border-ink flex flex-col">
            <span className="text-[12px] text-olive-wood-500 tracking-[2px] mb-5">{num}</span>
            <h3 className="text-[23px] font-black text-ink mb-4 tracking-[-0.5px]">{title}</h3>
            <p className="text-[15px] text-olive-wood-600 leading-[1.7]">{body}</p>
