  ...
  return (
    <section id="razon" className="bg-white px-6 md:px-16 py-[120px] grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-20 items-start">
      <div>
        ...
        <p className="text-[15px] leading-[1.75] text-olive-wood-500">
      ...
      <div className="flex flex-col">
        {ACTORES.map(({ num, label, desc }) => (
          <div key={num}
            ...
            <span className="text-[11px] text-olive-wood-300 font-semibold min-w-[24px]">{num}</span>
            <span className="text-[16px] font-bold text-ink flex-1">{label}</span>
            <span className="text-[13px] text-olive-wood-500">{desc}</span>
